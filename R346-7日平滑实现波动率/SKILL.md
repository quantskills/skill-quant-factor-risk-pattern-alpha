---
name: quant-real-factor-7d-smoothed-realized-volatility
description: Use when computing the 7D Smoothed Realized Volatility factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 7D Smoothed Realized Volatility

Use this Skill to compute `7日平滑实现波动率` / `7D Smoothed Realized Volatility` from caller-provided OHLCV data.
