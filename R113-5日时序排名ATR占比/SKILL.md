---
name: quant-real-factor-5d-time-series-ranked-atr-ratio
description: Use when computing the 5D Time-Series Ranked ATR Ratio factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Time-Series Ranked ATR Ratio

Use this Skill to compute `5日时序排名ATR占比` / `5D Time-Series Ranked ATR Ratio` from caller-provided OHLCV data.
