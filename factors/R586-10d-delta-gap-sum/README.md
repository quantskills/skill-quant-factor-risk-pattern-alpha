...
`10日变化缺口累积` 是一个框架中立的 OHLCV 因子 Skill，属于 `形态` 类。它只依赖用户传入的行情字段，不绑定任何交易框架或数据供应商。

### 因子逻辑

```text
delta(gap_sum, window=10)
```

该因子把“缺口累积”与“变化”处理结合，用来刻画价格、量能或波动状态在横截面上的可排序性。
...
- 市场覆盖: `{"cn": 98, "us": 50}`
- 覆盖率: `0.9910`
- 5日 Rank IC 均值: `0.001294`
- 5日 ICIR: `0.0467`
- 五分组 Q5-Q1 均值: `0.000264`
- Top 组换手: `0.5467`
- 无未来函数检查: `True`
- 状态: `pass`
...
`10D Delta Gap Sum` is a framework-neutral OHLCV factor Skill in the `Pattern` family. It depends only on caller-supplied market fields and is not tied to any trading framework or data vendor.
...
This factor combines Gap Sum with a Delta transform to test cross-sectional sortability in price, volume, or volatility states.
...
- Markets: `{"cn": 98, "us": 50}`
- Coverage: `0.9910`
- 5-day Rank IC mean: `0.001294`
- 5-day ICIR: `0.0467`
- Quintile Q5-Q1 mean: `0.000264`
- Top-quintile turnover: `0.5467`
