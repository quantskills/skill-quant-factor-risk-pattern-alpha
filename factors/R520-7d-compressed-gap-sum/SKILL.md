---
name: quant-real-factor-7d-compressed-gap-sum
description: Use when computing the 7D Compressed Gap Sum factor from user-supplied OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 7D Compressed Gap Sum

Use this Skill to compute `7日压缩缺口累积` / `7D Compressed Gap Sum` from caller-provided OHLCV data.
