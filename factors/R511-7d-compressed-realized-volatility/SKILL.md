---
name: quant-real-factor-7d-compressed-realized-volatility
description: Use when computing the 7D Compressed Realized Volatility factor from user-supplied OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 7D Compressed Realized Volatility

Use this Skill to compute `7日压缩实现波动率` / `7D Compressed Realized Volatility` from caller-provided OHLCV data.
