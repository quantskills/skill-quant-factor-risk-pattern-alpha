...
`10日量能确认随机区间位置` 是一个框架中立的 OHLCV 因子 Skill，属于 `震荡` 类。它只依赖用户传入的行情字段，不绑定任何交易框架或数据供应商。

### 因子逻辑

```text
confirm_volume(stoch, window=10)
```

该因子把“随机区间位置”与“量能确认”处理结合，用来刻画价格、量能或波动状态在横截面上的可排序性。
...
- 市场覆盖: `{"cn": 98, "us": 50}`
- 覆盖率: `0.9932`
- 5日 Rank IC 均值: `0.011865`
- 5日 ICIR: `0.4239`
- 五分组 Q5-Q1 均值: `0.001883`
- Top 组换手: `0.4363`
- 无未来函数检查: `True`
- 状态: `pass`
...
`10D Volume Confirmed Stochastic Position` is a framework-neutral OHLCV factor Skill in the `Oscillator` family. It depends only on caller-supplied market fields and is not tied to any trading framework or data vendor.
...
This factor combines Stochastic Position with a Volume Confirmed transform to test cross-sectional sortability in price, volume, or volatility states.
...
- Markets: `{"cn": 98, "us": 50}`
- Coverage: `0.9932`
- 5-day Rank IC mean: `0.011865`
- 5-day ICIR: `0.4239`
- Quintile Q5-Q1 mean: `0.001883`
- Top-quintile turnover: `0.4363`
