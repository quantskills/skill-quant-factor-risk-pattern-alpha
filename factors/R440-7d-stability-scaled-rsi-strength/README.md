...
`7日稳定度缩放RSI强度` 是一个使用真实行情验证过的 OHLCV 因子 Skill，属于 `震荡` 类。

### 因子逻辑

```text
stability(rsi, window=7)
```
...
`7D Stability Scaled RSI Strength` is a real-data-validated framework-neutral OHLCV factor Skill in the `Oscillator` family.
