---
name: quant-real-factor-5d-compressed-realized-volatility
description: Use when computing the 5D Compressed Realized Volatility factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Compressed Realized Volatility

Use this Skill to compute `5日压缩实现波动率` / `5D Compressed Realized Volatility` from caller-provided OHLCV data.
