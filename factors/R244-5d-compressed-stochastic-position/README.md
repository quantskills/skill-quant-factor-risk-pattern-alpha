...
`5日压缩随机区间位置` 是一个使用真实行情验证过的 OHLCV 因子 Skill，属于 `震荡` 类。

### 因子逻辑

```text
compress(stoch, window=5)
```

该因子将 `随机区间位置` 与 `压缩` 处理结合，观察价格、量能或波动状态在真实市场横截面中的可排序性。
...
`5D Compressed Stochastic Position` is a real-data-validated framework-neutral OHLCV factor Skill in the `Oscillator` family.
...
This factor combines `Stochastic Position` with a `Compressed` transform to test whether the resulting price, volume, or volatility state is cross-sectionally sortable in real market data.
...
- Usable rows: `196047`
- Coverage: `0.9955`
- 5-day Rank IC mean: `0.006597`
- 5-day ICIR: `0.2326`
- Quintile Q5-Q1 mean: `0.000641`
- Top-quintile turnover: `0.5295`
