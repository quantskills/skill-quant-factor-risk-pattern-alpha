---
name: quant-real-factor-5d-smoothed-downside-volatility
description: Use when computing the 5D Smoothed Downside Volatility factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Smoothed Downside Volatility

Use this Skill to compute `5日平滑下行波动率` / `5D Smoothed Downside Volatility` from caller-provided OHLCV data.
