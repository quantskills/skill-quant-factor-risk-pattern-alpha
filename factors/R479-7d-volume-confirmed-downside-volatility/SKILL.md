---
name: quant-real-factor-7d-volume-confirmed-downside-volatility
description: Use when computing the 7D Volume Confirmed Downside Volatility factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 7D Volume Confirmed Downside Volatility

Use this Skill to compute `7日量能确认下行波动率` / `7D Volume Confirmed Downside Volatility` from caller-provided OHLCV data.
