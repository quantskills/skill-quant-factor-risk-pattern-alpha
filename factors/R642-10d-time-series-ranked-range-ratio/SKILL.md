---
name: quant-real-factor-10d-time-series-ranked-range-ratio
description: Use when computing the 10D Time-Series Ranked Range Ratio factor from user-supplied OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 10D Time-Series Ranked Range Ratio

Use this Skill to compute `10日时序排名振幅占比` / `10D Time-Series Ranked Range Ratio` from caller-provided OHLCV data.
