---
name: quant-real-factor-5d-volume-confirmed-realized-volatility
description: Use when computing the 5D Volume Confirmed Realized Volatility factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Volume Confirmed Realized Volatility

Use this Skill to compute `5日量能确认实现波动率` / `5D Volume Confirmed Realized Volatility` from caller-provided OHLCV data.
