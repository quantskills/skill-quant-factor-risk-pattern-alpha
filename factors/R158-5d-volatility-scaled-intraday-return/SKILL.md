---
name: quant-real-factor-5d-volatility-scaled-intraday-return
description: Use when computing the 5D Volatility Scaled Intraday Return factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Volatility Scaled Intraday Return

Use this Skill to compute `5日波动缩放日内收益` / `5D Volatility Scaled Intraday Return` from caller-provided OHLCV data.
