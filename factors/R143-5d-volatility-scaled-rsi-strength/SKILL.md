---
name: quant-real-factor-5d-volatility-scaled-rsi-strength
description: Use when computing the 5D Volatility Scaled RSI Strength factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Volatility Scaled RSI Strength

Use this Skill to compute `5日波动缩放RSI强度` / `5D Volatility Scaled RSI Strength` from caller-provided OHLCV data.
