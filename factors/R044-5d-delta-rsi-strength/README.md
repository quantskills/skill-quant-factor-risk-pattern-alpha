...
`5日变化RSI强度` 是一个使用真实行情验证过的 OHLCV 因子 Skill，属于 `震荡` 类。
...
该因子将 `RSI强度` 与 `变化` 处理结合，观察价格、量能或波动状态在真实市场横截面中的可排序性。
...
`5D Delta RSI Strength` is a real-data-validated framework-neutral OHLCV factor Skill in the `Oscillator` family.
...
This factor combines `RSI Strength` with a `Delta` transform to test whether the resulting price, volume, or volatility state is cross-sectionally sortable in real market data.
...
- 5-day ICIR: `-0.2523`
- Quintile Q5-Q1 mean: `-0.000689`
- Top-quintile turnover: `0.8409`
