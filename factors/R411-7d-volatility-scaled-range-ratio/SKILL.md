---
name: quant-real-factor-7d-volatility-scaled-range-ratio
description: Use when computing the 7D Volatility Scaled Range Ratio factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 7D Volatility Scaled Range Ratio

Use this Skill to compute `7日波动缩放振幅占比` / `7D Volatility Scaled Range Ratio` from caller-provided OHLCV data.
