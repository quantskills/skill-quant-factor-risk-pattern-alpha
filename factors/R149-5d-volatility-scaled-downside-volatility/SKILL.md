---
name: quant-real-factor-5d-volatility-scaled-downside-volatility
description: Use when computing the 5D Volatility Scaled Downside Volatility factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Volatility Scaled Downside Volatility

Use this Skill to compute `5日波动缩放下行波动率` / `5D Volatility Scaled Downside Volatility` from caller-provided OHLCV data.
