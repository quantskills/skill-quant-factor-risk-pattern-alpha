---
name: quant-real-factor-5d-compressed-intraday-return
description: Use when computing the 5D Compressed Intraday Return factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Compressed Intraday Return

Use this Skill to compute `5日压缩日内收益` / `5D Compressed Intraday Return` from caller-provided OHLCV data.
