---
name: quant-real-factor-5d-time-series-ranked-gap-sum
description: Use when computing the 5D Time-Series Ranked Gap Sum factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Time-Series Ranked Gap Sum

Use this Skill to compute `5日时序排名缺口累积` / `5D Time-Series Ranked Gap Sum` from caller-provided OHLCV data.
