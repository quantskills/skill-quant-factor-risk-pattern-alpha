---
name: quant-real-factor-5d-volatility-scaled-range-ratio
description: Use when computing the 5D Volatility Scaled Range Ratio factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Volatility Scaled Range Ratio

Use this Skill to compute `5日波动缩放振幅占比` / `5D Volatility Scaled Range Ratio` from caller-provided OHLCV data.
