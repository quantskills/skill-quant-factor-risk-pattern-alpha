---
name: quant-real-factor-5d-time-series-ranked-intraday-return
description: Use when computing the 5D Time-Series Ranked Intraday Return factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Time-Series Ranked Intraday Return

Use this Skill to compute `5日时序排名日内收益` / `5D Time-Series Ranked Intraday Return` from caller-provided OHLCV data.
