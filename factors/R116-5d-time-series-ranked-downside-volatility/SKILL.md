---
name: quant-real-factor-5d-time-series-ranked-downside-volatility
description: Use when computing the 5D Time-Series Ranked Downside Volatility factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Time-Series Ranked Downside Volatility

Use this Skill to compute `5日时序排名下行波动率` / `5D Time-Series Ranked Downside Volatility` from caller-provided OHLCV data.
