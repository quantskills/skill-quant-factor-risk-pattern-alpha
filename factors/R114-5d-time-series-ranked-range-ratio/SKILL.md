---
name: quant-real-factor-5d-time-series-ranked-range-ratio
description: Use when computing the 5D Time-Series Ranked Range Ratio factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Time-Series Ranked Range Ratio

Use this Skill to compute `5日时序排名振幅占比` / `5D Time-Series Ranked Range Ratio` from caller-provided OHLCV data.
