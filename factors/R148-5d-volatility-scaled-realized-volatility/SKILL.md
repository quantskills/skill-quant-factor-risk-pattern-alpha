---
name: quant-real-factor-5d-volatility-scaled-realized-volatility
description: Use when computing the 5D Volatility Scaled Realized Volatility factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Volatility Scaled Realized Volatility

Use this Skill to compute `5日波动缩放实现波动率` / `5D Volatility Scaled Realized Volatility` from caller-provided OHLCV data.
