---
name: quant-real-factor-7d-volume-confirmed-realized-volatility
description: Use when computing the 7D Volume Confirmed Realized Volatility factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 7D Volume Confirmed Realized Volatility

Use this Skill to compute `7日量能确认实现波动率` / `7D Volume Confirmed Realized Volatility` from caller-provided OHLCV data.
