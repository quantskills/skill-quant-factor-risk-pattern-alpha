---
name: quant-real-factor-5d-time-series-ranked-realized-volatility
description: Use when computing the 5D Time-Series Ranked Realized Volatility factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Time-Series Ranked Realized Volatility

Use this Skill to compute `5日时序排名实现波动率` / `5D Time-Series Ranked Realized Volatility` from caller-provided OHLCV data.
