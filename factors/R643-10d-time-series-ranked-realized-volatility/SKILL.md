---
name: quant-real-factor-10d-time-series-ranked-realized-volatility
description: Use when computing the 10D Time-Series Ranked Realized Volatility factor from user-supplied OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 10D Time-Series Ranked Realized Volatility

Use this Skill to compute `10日时序排名实现波动率` / `10D Time-Series Ranked Realized Volatility` from caller-provided OHLCV data.
