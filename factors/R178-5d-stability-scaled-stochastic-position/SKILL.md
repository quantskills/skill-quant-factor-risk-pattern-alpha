---
name: quant-real-factor-5d-stability-scaled-stochastic-position
description: Use when computing the 5D Stability Scaled Stochastic Position factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Stability Scaled Stochastic Position

Use this Skill to compute `5日稳定度缩放随机区间位置` / `5D Stability Scaled Stochastic Position` from caller-provided OHLCV data.
