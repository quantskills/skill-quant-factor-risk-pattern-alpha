---
name: quant-real-factor-7d-volatility-scaled-gap-sum
description: Use when computing the 7D Volatility Scaled Gap Sum factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 7D Volatility Scaled Gap Sum

Use this Skill to compute `7日波动缩放缺口累积` / `7D Volatility Scaled Gap Sum` from caller-provided OHLCV data.
