---
name: quant-real-factor-7d-stability-scaled-intraday-return
description: Use when computing the 7D Stability Scaled Intraday Return factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 7D Stability Scaled Intraday Return

Use this Skill to compute `7日稳定度缩放日内收益` / `7D Stability Scaled Intraday Return` from caller-provided OHLCV data.
