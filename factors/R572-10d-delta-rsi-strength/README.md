...
`10日变化RSI强度` 是一个框架中立的 OHLCV 因子 Skill，属于 `震荡` 类。它只依赖用户传入的行情字段，不绑定任何交易框架或数据供应商。
...
该因子把“RSI强度”与“变化”处理结合，用来刻画价格、量能或波动状态在横截面上的可排序性。
...
`10D Delta RSI Strength` is a framework-neutral OHLCV factor Skill in the `Oscillator` family. It depends only on caller-supplied market fields and is not tied to any trading framework or data vendor.
...
This factor combines RSI Strength with a Delta transform to test cross-sectional sortability in price, volume, or volatility states.
