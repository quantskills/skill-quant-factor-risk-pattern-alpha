---
name: quant-real-factor-7d-smoothed-gap-sum
description: Use when computing the 7D Smoothed Gap Sum factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
Use this Skill to compute `7日平滑缺口累积` / `7D Smoothed Gap Sum` from caller-provided OHLCV data.
