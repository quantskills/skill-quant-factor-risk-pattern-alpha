---
name: quant-real-factor-7d-smoothed-intraday-return
description: Use when computing the 7D Smoothed Intraday Return factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 7D Smoothed Intraday Return

Use this Skill to compute `7日平滑日内收益` / `7D Smoothed Intraday Return` from caller-provided OHLCV data.
