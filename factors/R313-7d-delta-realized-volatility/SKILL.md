---
name: quant-real-factor-7d-delta-realized-volatility
description: Use when computing the 7D Delta Realized Volatility factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 7D Delta Realized Volatility

Use this Skill to compute `7日变化实现波动率` / `7D Delta Realized Volatility` from caller-provided OHLCV data.
