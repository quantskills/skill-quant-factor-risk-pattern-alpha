---
name: quant-real-factor-5d-time-series-ranked-rsi-reversal
description: Use when computing the 5D Time-Series Ranked RSI Reversal factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Time-Series Ranked RSI Reversal

Use this Skill to compute `5日时序排名RSI反转` / `5D Time-Series Ranked RSI Reversal` from caller-provided OHLCV data.
