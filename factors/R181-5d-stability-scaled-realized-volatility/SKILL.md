---
name: quant-real-factor-5d-stability-scaled-realized-volatility
description: Use when computing the 5D Stability Scaled Realized Volatility factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Stability Scaled Realized Volatility

Use this Skill to compute `5日稳定度缩放实现波动率` / `5D Stability Scaled Realized Volatility` from caller-provided OHLCV data.
