...
`5日稳定度缩放RSI反转` 是一个使用真实行情验证过的 OHLCV 因子 Skill，属于 `震荡` 类。

### 因子逻辑

```text
stability(rsi_reversal, window=5)
```
...
`5D Stability Scaled RSI Reversal` is a real-data-validated framework-neutral OHLCV factor Skill in the `Oscillator` family.
...
- 5-day ICIR: `-0.3398`
- Quintile Q5-Q1 mean: `-0.001536`
- Top-quintile turnover: `0.3563`
