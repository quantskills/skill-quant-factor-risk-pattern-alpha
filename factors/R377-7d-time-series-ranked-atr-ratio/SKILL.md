---
name: quant-real-factor-7d-time-series-ranked-atr-ratio
description: Use when computing the 7D Time-Series Ranked ATR Ratio factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 7D Time-Series Ranked ATR Ratio

Use this Skill to compute `7日时序排名ATR占比` / `7D Time-Series Ranked ATR Ratio` from caller-provided OHLCV data.
