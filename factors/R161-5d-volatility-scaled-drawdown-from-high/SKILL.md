---
name: quant-real-factor-5d-volatility-scaled-drawdown-from-high
description: Use when computing the 5D Volatility Scaled Drawdown From High factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Volatility Scaled Drawdown From High

Use this Skill to compute `5日波动缩放距高点回撤` / `5D Volatility Scaled Drawdown From High` from caller-provided OHLCV data.
