---
name: quant-real-factor-5d-z-scored-realized-volatility
description: Use when computing the 5D Z-Scored Realized Volatility factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Z-Scored Realized Volatility

Use this Skill to compute `5日标准化实现波动率` / `5D Z-Scored Realized Volatility` from caller-provided OHLCV data.
