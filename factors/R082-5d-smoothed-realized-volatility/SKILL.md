---
name: quant-real-factor-5d-smoothed-realized-volatility
description: Use when computing the 5D Smoothed Realized Volatility factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Smoothed Realized Volatility

Use this Skill to compute `5日平滑实现波动率` / `5D Smoothed Realized Volatility` from caller-provided OHLCV data.
