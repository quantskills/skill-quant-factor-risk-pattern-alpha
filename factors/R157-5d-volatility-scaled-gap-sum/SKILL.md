---
name: quant-real-factor-5d-volatility-scaled-gap-sum
description: Use when computing the 5D Volatility Scaled Gap Sum factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Volatility Scaled Gap Sum

Use this Skill to compute `5日波动缩放缺口累积` / `5D Volatility Scaled Gap Sum` from caller-provided OHLCV data.
