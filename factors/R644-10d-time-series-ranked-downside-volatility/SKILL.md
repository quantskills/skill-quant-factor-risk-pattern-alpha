---
name: quant-real-factor-10d-time-series-ranked-downside-volatility
description: Use when computing the 10D Time-Series Ranked Downside Volatility factor from user-supplied OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 10D Time-Series Ranked Downside Volatility

Use this Skill to compute `10日时序排名下行波动率` / `10D Time-Series Ranked Downside Volatility` from caller-provided OHLCV data.
