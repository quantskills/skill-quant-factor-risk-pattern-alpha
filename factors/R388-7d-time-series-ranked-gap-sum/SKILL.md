---
name: quant-real-factor-7d-time-series-ranked-gap-sum
description: Use when computing the 7D Time-Series Ranked Gap Sum factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 7D Time-Series Ranked Gap Sum

Use this Skill to compute `7日时序排名缺口累积` / `7D Time-Series Ranked Gap Sum` from caller-provided OHLCV data.
