...
`10日平滑RSI反转` 是一个框架中立的 OHLCV 因子 Skill，属于 `震荡` 类。它只依赖用户传入的行情字段，不绑定任何交易框架或数据供应商。

### 因子逻辑

```text
smooth(rsi_reversal, window=10)
```

该因子把“RSI反转”与“平滑”处理结合，用来刻画价格、量能或波动状态在横截面上的可排序性。
...
- 市场覆盖: `{"cn": 98, "us": 50}`
- 覆盖率: `0.9916`
- 5日 Rank IC 均值: `-0.014638`
- 5日 ICIR: `-0.4888`
- 五分组 Q5-Q1 均值: `-0.002410`
- Top 组换手: `0.1742`
- 无未来函数检查: `True`
- 状态: `pass`
...
`10D Smoothed RSI Reversal` is a framework-neutral OHLCV factor Skill in the `Oscillator` family. It depends only on caller-supplied market fields and is not tied to any trading framework or data vendor.
...
This factor combines RSI Reversal with a Smoothed transform to test cross-sectional sortability in price, volume, or volatility states.
...
- Markets: `{"cn": 98, "us": 50}`
- Coverage: `0.9916`
- 5-day Rank IC mean: `-0.014638`
- 5-day ICIR: `-0.4888`
- Quintile Q5-Q1 mean: `-0.002410`
- Top-quintile turnover: `0.1742`
