---
name: quant-real-factor-5d-delta-realized-volatility
description: Use when computing the 5D Delta Realized Volatility factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Delta Realized Volatility

Use this Skill to compute `5日变化实现波动率` / `5D Delta Realized Volatility` from caller-provided OHLCV data.
