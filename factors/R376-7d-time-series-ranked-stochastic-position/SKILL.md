---
name: quant-real-factor-7d-time-series-ranked-stochastic-position
description: Use when computing the 7D Time-Series Ranked Stochastic Position factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 7D Time-Series Ranked Stochastic Position

Use this Skill to compute `7日时序排名随机区间位置` / `7D Time-Series Ranked Stochastic Position` from caller-provided OHLCV data.
