...
`7日压缩振幅占比` 是一个框架中立的 OHLCV 因子 Skill，属于 `波动率` 类。它只依赖用户传入的行情字段，不绑定任何交易框架或数据供应商。

### 因子逻辑

```text
compress(range_ratio, window=7)
```
...
- 市场覆盖: `{"cn": 98, "us": 50}`
- 覆盖率: `0.9947`
- 5日 Rank IC 均值: `-0.042431`
- 5日 ICIR: `-1.3162`
- 五分组 Q5-Q1 均值: `0.000638`
- Top 组换手: `0.1382`
- 无未来函数检查: `True`
- 状态: `pass`
...
`7D Compressed Range Ratio` is a framework-neutral OHLCV factor Skill in the `Volatility` family. It depends only on caller-supplied market fields and is not tied to any trading framework or data vendor.
...
- Markets: `{"cn": 98, "us": 50}`
- Coverage: `0.9947`
- 5-day Rank IC mean: `-0.042431`
- 5-day ICIR: `-1.3162`
- Quintile Q5-Q1 mean: `0.000638`
- Top-quintile turnover: `0.1382`
