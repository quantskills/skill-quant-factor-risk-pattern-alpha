---
name: quant-real-factor-7d-volatility-scaled-downside-volatility
description: Use when computing the 7D Volatility Scaled Downside Volatility factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 7D Volatility Scaled Downside Volatility

Use this Skill to compute `7日波动缩放下行波动率` / `7D Volatility Scaled Downside Volatility` from caller-provided OHLCV data.
