---
name: quant-real-factor-7d-time-series-ranked-range-ratio
description: Use when computing the 7D Time-Series Ranked Range Ratio factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 7D Time-Series Ranked Range Ratio

Use this Skill to compute `7日时序排名振幅占比` / `7D Time-Series Ranked Range Ratio` from caller-provided OHLCV data.
