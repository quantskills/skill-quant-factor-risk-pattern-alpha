---
name: quant-real-factor-5d-time-series-ranked-stochastic-position
description: Use when computing the 5D Time-Series Ranked Stochastic Position factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Time-Series Ranked Stochastic Position

Use this Skill to compute `5日时序排名随机区间位置` / `5D Time-Series Ranked Stochastic Position` from caller-provided OHLCV data.
