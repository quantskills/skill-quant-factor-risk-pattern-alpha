---
name: quant-real-factor-5d-volume-confirmed-gap-sum
description: Use when computing the 5D Volume Confirmed Gap Sum factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Volume Confirmed Gap Sum

Use this Skill to compute `5日量能确认缺口累积` / `5D Volume Confirmed Gap Sum` from caller-provided OHLCV data.
