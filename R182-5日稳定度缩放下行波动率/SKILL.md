---
name: quant-real-factor-5d-stability-scaled-downside-volatility
description: Use when computing the 5D Stability Scaled Downside Volatility factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Stability Scaled Downside Volatility

Use this Skill to compute `5日稳定度缩放下行波动率` / `5D Stability Scaled Downside Volatility` from caller-provided OHLCV data.
