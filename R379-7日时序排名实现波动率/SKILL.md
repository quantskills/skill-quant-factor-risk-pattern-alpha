---
name: quant-real-factor-7d-time-series-ranked-realized-volatility
description: Use when computing the 7D Time-Series Ranked Realized Volatility factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 7D Time-Series Ranked Realized Volatility

Use this Skill to compute `7日时序排名实现波动率` / `7D Time-Series Ranked Realized Volatility` from caller-provided OHLCV data.
