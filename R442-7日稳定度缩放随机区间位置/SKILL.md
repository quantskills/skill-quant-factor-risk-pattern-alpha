---
name: quant-real-factor-7d-stability-scaled-stochastic-position
description: Use when computing the 7D Stability Scaled Stochastic Position factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 7D Stability Scaled Stochastic Position

Use this Skill to compute `7日稳定度缩放随机区间位置` / `7D Stability Scaled Stochastic Position` from caller-provided OHLCV data.
