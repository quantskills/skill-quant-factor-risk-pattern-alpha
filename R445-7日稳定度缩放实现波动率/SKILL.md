---
name: quant-real-factor-7d-stability-scaled-realized-volatility
description: Use when computing the 7D Stability Scaled Realized Volatility factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 7D Stability Scaled Realized Volatility

Use this Skill to compute `7日稳定度缩放实现波动率` / `7D Stability Scaled Realized Volatility` from caller-provided OHLCV data.
