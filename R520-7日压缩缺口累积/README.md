...
`7日压缩缺口累积` 是一个框架中立的 OHLCV 因子 Skill，属于 `形态` 类。它只依赖用户传入的行情字段，不绑定任何交易框架或数据供应商。

### 因子逻辑

```text
compress(gap_sum, window=7)
```
...
`7D Compressed Gap Sum` is a framework-neutral OHLCV factor Skill in the `Pattern` family. It depends only on caller-supplied market fields and is not tied to any trading framework or data vendor.
...
- Markets: `{"cn": 98, "us": 50}`
- Coverage: `0.9940`
- 5-day Rank IC mean: `0.013356`
- 5-day ICIR: `0.4964`
- Quintile Q5-Q1 mean: `0.000668`
- Top-quintile turnover: `0.2933`
