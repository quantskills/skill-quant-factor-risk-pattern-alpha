---
name: quant-real-factor-7d-volatility-scaled-realized-volatility
description: Use when computing the 7D Volatility Scaled Realized Volatility factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 7D Volatility Scaled Realized Volatility

Use this Skill to compute `7日波动缩放实现波动率` / `7D Volatility Scaled Realized Volatility` from caller-provided OHLCV data.
