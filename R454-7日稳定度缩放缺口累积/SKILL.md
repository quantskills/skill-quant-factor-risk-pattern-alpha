---
name: quant-real-factor-7d-stability-scaled-gap-sum
description: Use when computing the 7D Stability Scaled Gap Sum factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
Use this Skill to compute `7日稳定度缩放缺口累积` / `7D Stability Scaled Gap Sum` from caller-provided OHLCV data.
