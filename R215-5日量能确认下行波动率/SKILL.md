---
name: quant-real-factor-5d-volume-confirmed-downside-volatility
description: Use when computing the 5D Volume Confirmed Downside Volatility factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Volume Confirmed Downside Volatility

Use this Skill to compute `5日量能确认下行波动率` / `5D Volume Confirmed Downside Volatility` from caller-provided OHLCV data.
