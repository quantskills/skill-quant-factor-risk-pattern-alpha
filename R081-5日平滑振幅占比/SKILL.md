---
name: quant-real-factor-5d-smoothed-range-ratio
description: Use when computing the 5D Smoothed Range Ratio factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Smoothed Range Ratio

Use this Skill to compute `5日平滑振幅占比` / `5D Smoothed Range Ratio` from caller-provided OHLCV data.
