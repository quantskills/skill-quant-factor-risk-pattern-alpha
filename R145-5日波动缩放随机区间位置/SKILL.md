---
name: quant-real-factor-5d-volatility-scaled-stochastic-position
description: Use when computing the 5D Volatility Scaled Stochastic Position factor from user-supplied real OHLCV data or reviewing its bundled real-data validation metrics.
metadata:
...
# 5D Volatility Scaled Stochastic Position

Use this Skill to compute `5日波动缩放随机区间位置` / `5D Volatility Scaled Stochastic Position` from caller-provided OHLCV data.
